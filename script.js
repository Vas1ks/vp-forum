// ========== ССЫЛКИ ==========
  ...
  async function loadData() {
    try {
      ...
      console.error(e);
      document.getElementById('app').innerHTML = '<div style="padding:40px;text-align:center;">⚠️ Ошибка загрузки</div>'; 
    }
  }
  ...
  async function toggleLike(commentId, userNick) {
    ...
    try {
      ...
      // Обновляем локальный массив лайков
      if(alreadyLiked) {
  ...
  window.likeComment = async (commentId) => {
    ...
    try {
      ...
      // Находим кнопку лайка и обновляем её счётчик и класс
